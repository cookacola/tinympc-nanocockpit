...
void gap8_BNReluConvolution3(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      x_tile_size_nif = (_i_nif_load+1 == 2) ? 8 : 16;
      x_tile_size_h   = (_i_h_load+1 == 4)   ? 13 : 17;
      x_tile_size_w   = (_i_w_load+1 == 1)   ? 80 : 80;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
      x_length_nif_byte = (_i_nif_load+1 == 2)   ? 8 : 16;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 4)   ? 6 : 8;
      y_tile_size_w   = (_i_w_load+1 == 1)   ? 40 : 40;
      y_tile_size_nof = (_i_nof_load+1 == 2) ? 8 : 16;
      y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
      y_length_nof_byte = (_i_nof_load+1 == 2)   ? 8 : 16;
      W_tile_size_nof = (_i_nof_load+1 == 2) ? 8 : 16;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 17, 80, 16, 80, 24,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+216 + 128*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 27048;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+408 + 128*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 27184;
    ...
    k = (int64_t *) (l1_buffer + 27048);
    lambda = (int64_t *) (l1_buffer + 27184);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 8, 40, 16, 40, 24, 0, 0, 0, 0, 0, 0, 8);
