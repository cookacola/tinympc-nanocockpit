...
void gap8_BNReluConvolution7(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 17, 40, 32, 40, 48,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+432 + 256*_i_nof_load;
        ...
        DMA_copy_lambda.ext = (uint32_t) l2_W+816 + 256*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 27456;
    ...
    k = (int64_t *) (l1_buffer + 27192);
    lambda = (int64_t *) (l1_buffer + 27456);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 8, 20, 32, 20, 48, 0, 0, 0, 0, 0, 0, 8);
