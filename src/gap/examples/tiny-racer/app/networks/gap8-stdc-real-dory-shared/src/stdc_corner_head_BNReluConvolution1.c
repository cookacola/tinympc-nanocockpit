...
void stdc_corner_head_BNReluConvolution1(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 6 : 24;
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 10 : 30;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 0;
      y_tile_size_h   = (_i_h_load+1 == 2)   ? 6 : 24;
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 10 : 30;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 24, 30, 32, 40, 32,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 16, 1*1, 32, 1*1, 32, 0,0,0,0,0,0, 8);
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+512 + 64*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 35096;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+576 + 64*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 35168;
    ...
    k = (int32_t *) (l1_buffer + 35096);
    lambda = (int32_t *) (l1_buffer + 35168);
    W = (uint8_t *) (l1_buffer + 34576);
    y = (uint8_t *) (l1_buffer + 23048);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 24, 30, 16, 40, 16, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 23048);
