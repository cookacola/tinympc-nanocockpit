...
void stdc_encoder_BNReluConvolution6(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 6 : 34;
      ...
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 6 : 34;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 16, 34, 32, 40, 32,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 32, 1*1, 32, 1*1, 32, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 34832);
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+1024 + 128*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 35864;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+1152 + 128*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 36000;
    ...
    k = (int32_t *) (l1_buffer + 35864);
    lambda = (int32_t *) (l1_buffer + 36000);
    W = (uint8_t *) (l1_buffer + 34832);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 16, 34, 32, 40, 32, 0, 0, 0, 0, 0, 0, 8);
