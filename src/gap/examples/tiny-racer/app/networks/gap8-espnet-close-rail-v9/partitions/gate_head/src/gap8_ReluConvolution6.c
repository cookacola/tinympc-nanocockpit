...
#include "gap8_ReluConvolution6.h"
#include "pulp.h"
#include "pmsis.h"
...
void gap8_ReluConvolution6(
  ...
  DMA_copy_W.number_of_2d_copies = 6;
  DMA_copy_W.number_of_1d_copies = 1;
  DMA_copy_W.dir = 1;
  DMA_copy_W.tid = dory_dma_channel;
  
  DMA_copy_y.hwc_to_chw = 0;
  DMA_copy_y.stride_2d = 6;
  DMA_copy_y.stride_1d = 6;
  DMA_copy_y.dir = 0;
  ...
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+576;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 718);
  DMA_copy_bias.number_of_2d_copies = 1;
  DMA_copy_bias.number_of_1d_copies = 1;
  DMA_copy_bias.length_1d_copy = (uint16_t) 24;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      y_tile_size_nof = (_i_nof_load+1 == 1) ? 6 : 6;
      y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
      y_length_nof_byte = (_i_nof_load+1 == 1)   ? 6 : 6;
      W_tile_size_nof = (_i_nof_load+1 == 1) ? 6 : 6;
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 6, 1*1, 96, 1*1, 96, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 118);
    ...
    b = (uint8_t *) (l1_buffer + 718 + _i_nof_load*24);
    W = (uint8_t *) (l1_buffer + 118);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 1, 1, 6, 1, 6, 0, 0, 0, 0, 0, 0, 8);
