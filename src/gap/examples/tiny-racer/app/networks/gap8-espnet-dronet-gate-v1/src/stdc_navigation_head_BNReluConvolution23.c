...
void stdc_navigation_head_BNReluConvolution23(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  DMA_copy_W.number_of_2d_copies = 2;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      y_tile_size_nof = (_i_nof_load+1 == 1) ? 2 : 2;
      y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
      y_length_nof_byte = (_i_nof_load+1 == 1)   ? 2 : 2;
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 2, 1*1, 96, 1*1, 96, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 9816);
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+192 + 16*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 10016;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+208 + 16*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 10040;
    ...
    k = (int64_t *) (l1_buffer + 10016);
    lambda = (int64_t *) (l1_buffer + 10040);
    W = (uint8_t *) (l1_buffer + 9816);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 10, 10, 2, 10, 2, 0, 0, 0, 0, 0, 0, 8);
