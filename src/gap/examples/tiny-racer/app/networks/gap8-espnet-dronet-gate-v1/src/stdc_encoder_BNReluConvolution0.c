...
void stdc_encoder_BNReluConvolution0(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      x_tile_size_nif = (_i_nif_load+1 == 1) ? 2 : 2;
      x_tile_size_h   = (_i_h_load+1 == 3)   ? 33 : 65;
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 69 : 93;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
      x_length_nif_byte = (_i_nif_load+1 == 1)   ? 2 : 2;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 3)   ? 16 : 32;
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 34 : 46;
      ...
      W_tile_size_nif = (_i_nif_load+1 == 1) ? 2 : 2;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 65, 93, 2, 160, 2,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 16, 3*3, 2, 3*3, 2, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 35658);
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+288 + 128*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 35954;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+416 + 128*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 36090;
    ...
    k = (int64_t *) (l1_buffer + 35954);
    lambda = (int64_t *) (l1_buffer + 36090);
    W = (uint8_t *) (l1_buffer + 35658);
    y = (uint8_t *) (l1_buffer + 12098);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 32, 46, 16, 80, 16, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 12098);
