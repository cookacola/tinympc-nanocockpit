...
void stdc_corner_head_BNReluConvolution1(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 20, 20, 32, 20, 32,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+288 + 256*_i_nof_load;
        ...
        DMA_copy_lambda.ext = (uint32_t) l2_W+544 + 256*_i_nof_load;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 20, 20, 32, 20, 32, 0, 0, 0, 0, 0, 0, 8);
