...
#include "mem.h"
#include <string.h>
#include "stdc_corner_head_BNReluConvolution3.h"
#include "stdc_corner_head_BNReluConvolution1.h"
#include "stdc_corner_head_BNReluConvolution0.h"
#include "stdc_corner_head_BNReluConvolution2.h"
...
int stdc_corner_head_cycle_network_execution;
...
void stdc_corner_head_execute_layer_fork(void *args) {
  ...
  {
    case 0:
      pi_cl_team_fork(NUM_CORES, (void *)stdc_corner_head_BNReluConvolution0, args);
      break;
    case 1:
      pi_cl_team_fork(NUM_CORES, (void *)stdc_corner_head_BNReluConvolution1, args);
      break;
    case 2:
      pi_cl_team_fork(NUM_CORES, (void *)stdc_corner_head_BNReluConvolution2, args);
      break;
    case 3:
      pi_cl_team_fork(NUM_CORES, (void *)stdc_corner_head_BNReluConvolution3, args);
      break;
  }
...
struct stdc_corner_head_network_run_token stdc_corner_head_network_run_async(void *l2_buffer, size_t l2_buffer_size, void *l2_final_output, int exec, int initial_dir)
{
  ...
  // open cluster...
  pi_cluster_task(&cluster_task, stdc_corner_head_network_run_cluster, args);
  ...
  return (struct stdc_corner_head_network_run_token) {
...
void stdc_corner_head_network_run_wait(struct stdc_corner_head_network_run_token token)
{
  ...
  print_perf("Final", stdc_corner_head_cycle_network_execution, 1164800);
}

void stdc_corner_head_network_run(void *l2_buffer, size_t l2_buffer_size, void *l2_final_output, int exec, int initial_dir)
{
  stdc_corner_head_network_run_wait(stdc_corner_head_network_run_async(l2_buffer, l2_buffer_size, l2_final_output, exec, initial_dir));
}
...
static struct pi_cluster_task stdc_corner_head_cluster_task_async;
static unsigned int stdc_corner_head_args_async[5];
...
{
  stdc_corner_head_args_async[0] = (unsigned int) l2_buffer;
  stdc_corner_head_args_async[1] = (unsigned int) l2_buffer_size;
  stdc_corner_head_args_async[2] = (unsigned int) l2_final_output;
  stdc_corner_head_args_async[3] = (unsigned int) exec;
  stdc_corner_head_args_async[4] = (unsigned int) initial_dir;
  pi_cluster_task(&stdc_corner_head_cluster_task_async, stdc_corner_head_network_run_cluster, stdc_corner_head_args_async);
  stdc_corner_head_cluster_task_async.stack_size = 3500;
  stdc_corner_head_cluster_task_async.slave_stack_size = 3400;
  pi_cluster_send_task_to_cl_async(cluster, &stdc_corner_head_cluster_task_async, network_done);
}
...
/* ---------------------------------- */
  ...
  stdc_corner_head_cycle_network_execution = 0;
/* MAIN SECTION
...
*/
    ...
    pi_perf_stop();
    pi_perf_start();
    stdc_corner_head_execute_layer_fork((void *) &largs);
    // performance measurements: end
    pi_perf_stop();
    perf_cyc =  pi_perf_read(PI_PERF_CYCLES);
    stdc_corner_head_cycle_network_execution += perf_cyc;
  ...
  //memcpy(L2_output, l2_final_output, activations_out_size[3]); // BUGGY!
  for (int i=0; i<activations_out_size[3]; i++)
