...
void stdc_presence_head_BNReluConvolution0(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 20, 20, 64, 20, 64,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 1, 1*1, 64, 1*1, 64, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 26016);
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+64 + 8*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 26088;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+72 + 8*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 26104;
    ...
    k = (int64_t *) (l1_buffer + 26088);
    lambda = (int64_t *) (l1_buffer + 26104);
    W = (uint8_t *) (l1_buffer + 26016);
