...
void stdc_navigation_head_BNReluConvolution17(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+864 + 768*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 20088;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+1632 + 768*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 20864;
    ...
    k = (int64_t *) (l1_buffer + 20088);
    lambda = (int64_t *) (l1_buffer + 20864);
