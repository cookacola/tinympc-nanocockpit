...
int32_t himax_init(himax_t *himax) {
    ...
    pi_pad_set_function(PI_PAD_31_B11_TIMER0_CH0, PI_PAD_FUNC0);
    pi_pwm_timer_start(&himax->mclk_timer);
    uint32_t pwm_first = *(volatile uint32_t *)(ARCHI_PWM_ADDR + 0x2c);
    pi_time_wait_us(123);
    VERBOSE_PRINT("CAMERA PWM count=%lu,%lu cfg=%08lx threshold=%08lx enable=%08lx\n",
        (unsigned long)pwm_first, (unsigned long)*(volatile uint32_t *)(ARCHI_PWM_ADDR + 0x2c),
        (unsigned long)*(volatile uint32_t *)(ARCHI_PWM_ADDR + 4),
        (unsigned long)*(volatile uint32_t *)(ARCHI_PWM_ADDR + 8),
        (unsigned long)*(volatile uint32_t *)(ARCHI_PWM_ADDR + 0x104));
    ...
    camera_conf.format = HIMAX_WIDTH;
