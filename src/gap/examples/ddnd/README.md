...
Physical deployment check (2026-09-11): flashed through the Ubuntu VM/Olimex and verified compact packets over Crazyradio after a full battery disconnect/reconnect, with Olimex disconnected. Six packets arrived with zero CRC/invalid errors, status 3, and approximately 1.49 s inference; all motor outputs were zero. See `cold_boot_verification.json`. The radio-triggered `stm_power_cycle()` did not restore packets in this test; use a full battery power cycle after flashing. No flight was performed.
