...
void FullyConnected8(
  ...
  im2col = l1_buffer + 17384;
  uint16_t out_shift = out_shift_in;

  ////////////////////////////
  // First tile transfering //
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+15360;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 17352);
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 1968) + 0;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_W = !db_state_W ? 15360 : 0;
    db_y = !db_state_y ? 32 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 15360 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 8, 1*1, 1920, 1*1, 1920, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 1968) + db_W;
    ...
    b = (uint8_t *) (l1_buffer + 17352 + _i_nof_exec*32);
    W = (uint8_t *) (l1_buffer + 1968 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 1928 + db_y);
   ...
   // wait for DMA write/read
     ...
     dory_dma_barrier(&DMA_copy_W);
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 1, 1, 8, 1, 8, 0, 0, 0, 0, 0, 0, 32);
