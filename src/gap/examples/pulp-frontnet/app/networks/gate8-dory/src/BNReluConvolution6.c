...
void BNReluConvolution6(
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+73728;
  DMA_copy_k.loc = (uint32_t) l1_buffer + 24472;
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+74752;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 24736;
  ...
  DMA_copy_x.number_of_2d_copies = 5;
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 6032) + 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      W_tile_size_nof = (_i_nof_load+1 == 8) ? 16 : 16;
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 16, 3*3, 64, 3*3, 64, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 6032) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+73728 + 128*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 24472 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+74752 + 128*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 24736 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 24472 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 24736 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 6032 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 5768 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 64 : 64;
    x_tile_size_h_exec   = (_i_h_exec+1 == 2)   ? 3 : 5;
    x_tile_size_w_exec   = (_i_w_exec+1 == 2)   ? 3 : 9;
    y_tile_size_nof = (_i_nof_exec+1 == 8) ? 16 : 16;
    y_tile_size_h   = (_i_h_exec+1 == 2)   ? 1 : 2;
    y_tile_size_w   = (_i_w_exec+1 == 2)   ? 1 : 4;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (_i_nof_exec+1 == 8)   ? 16 : 16;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 2, 4, 16, 5, 128, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 5768) + db_y;
