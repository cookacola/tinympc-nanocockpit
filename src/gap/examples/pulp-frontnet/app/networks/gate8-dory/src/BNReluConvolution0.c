...
void BNReluConvolution0(
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+800;
  DMA_copy_k.loc = (uint32_t) l1_buffer + 33490;
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+1056;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 34010;
  ...
  DMA_copy_x.number_of_2d_copies = 19;
  DMA_copy_x.number_of_1d_copies = 111;
  DMA_copy_x.length_1d_copy = 1;
  dory_dma_memcpy_async(&DMA_copy_x);
  dory_dma_barrier(&DMA_copy_x);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 31882) + 0;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 2109 : 0;
    db_W = !db_state_W ? 800 : 0;
    db_y = !db_state_y ? 13824 : 0;
    db_act = !db_state_W ? 256 : 0;
    exec_db_x = db_state_x ? 2109 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 6)   ? 18 : 19;
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 54 : 111;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 2;
      y_tile_size_h   = (_i_h_load+1 == 6)   ? 8 : 8;
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 26 : 54;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 19, 111, 1, 160, 1,  3, 3,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_W.loc = (l1_buffer + 31882) + db_W;
    ...
    k = (int64_t *) (l1_buffer + 33490 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 34010 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 31882 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 4226 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 1 : 1;
    x_tile_size_h_exec   = (_i_h_exec+1 == 6)   ? 18 : 19;
    x_tile_size_w_exec   = (_i_w_exec+1 == 2)   ? 54 : 111;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 32 : 32;
    y_tile_size_h   = (_i_h_exec+1 == 6)   ? 8 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 2)   ? 26 : 54;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 54, 32, 80, 32, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 4226) + db_y;
