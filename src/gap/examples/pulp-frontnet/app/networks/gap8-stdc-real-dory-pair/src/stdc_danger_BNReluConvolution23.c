...
void stdc_danger_BNReluConvolution23(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 10, 64, 10, 64,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 96, 1*1, 64, 1*1, 64, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 12816);
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+6144 + 384*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 18968;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+6528 + 384*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 19360;
    ...
    k = (int32_t *) (l1_buffer + 18968);
    lambda = (int32_t *) (l1_buffer + 19360);
    W = (uint8_t *) (l1_buffer + 12816);
