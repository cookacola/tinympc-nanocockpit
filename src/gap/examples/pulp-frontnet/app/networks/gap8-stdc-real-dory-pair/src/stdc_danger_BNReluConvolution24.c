...
void stdc_danger_BNReluConvolution24(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 10, 96, 10, 96,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.loc = (uint32_t) l1_buffer + 16248;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+1248 + 384*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 16640;
    ...
    k = (int32_t *) (l1_buffer + 16248);
    lambda = (int32_t *) (l1_buffer + 16640);
