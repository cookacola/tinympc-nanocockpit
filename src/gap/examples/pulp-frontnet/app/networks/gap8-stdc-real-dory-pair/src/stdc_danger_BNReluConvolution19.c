...
void stdc_danger_BNReluConvolution19(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  im2col = l1_buffer + 29656;
  volatile uint8_t *pwt_buffer;
  pwt_buffer = im2col + 432;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      x_tile_size_nif = (_i_nif_load+1 == 2) ? 16 : 48;
      x_tile_size_h   = (_i_h_load+1 == 1)   ? 15 : 15;
      x_tile_size_w   = (_i_w_load+1 == 1)   ? 20 : 20;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
      x_length_nif_byte = (_i_nif_load+1 == 2)   ? 16 : 48;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 15, 20, 48, 20, 64,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+576 + 192*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 29256;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+832 + 192*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 29456;
    ...
    k = (int32_t *) (l1_buffer + 29256);
    lambda = (int32_t *) (l1_buffer + 29456);
    W = (uint8_t *) (l1_buffer + 28816);
    y = (uint8_t *) (l1_buffer + 14408);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 15, 20, 48, 20, 64, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 14408);
