...
void gap8_BNReluConvolution5(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      x_tile_size_nif = (_i_nif_load+1 == 2) ? 4 : 16;
      ...
      x_length_nif_byte = (_i_nif_load+1 == 2)   ? 4 : 16;
      ...
      y_tile_size_nof = (_i_nof_load+1 == 2) ? 4 : 16;
      y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
      y_length_nof_byte = (_i_nof_load+1 == 2)   ? 4 : 16;
      W_tile_size_nof = (_i_nof_load+1 == 2) ? 4 : 16;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 26, 40, 16, 40, 20,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+180 + 64*_i_nof_load;
        ...
        DMA_copy_lambda.ext = (uint32_t) l2_W+260 + 64*_i_nof_load;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 24, 40, 16, 40, 20, 0, 0, 0, 0, 0, 0, 8);
