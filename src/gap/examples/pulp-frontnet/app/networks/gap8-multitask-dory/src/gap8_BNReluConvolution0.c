...
void gap8_BNReluConvolution0(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 65 : 97;
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 37 : 125;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 2)   ? 32 : 48;
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 18 : 62;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 97, 125, 1, 160, 1,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+72 + 32*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 36029;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+104 + 32*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 36069;
    ...
    k = (int32_t *) (l1_buffer + 36029);
    lambda = (int32_t *) (l1_buffer + 36069);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 48, 62, 8, 80, 8, 0, 0, 0, 0, 0, 0, 8);
