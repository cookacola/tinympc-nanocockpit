...
void gap8_BNReluConvolution3(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 4)   ? 9 : 25;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 4)   ? 4 : 12;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 25, 80, 12, 80, 12,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 12, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 29776);
        DMA_copy_W.length_1d_copy = (int) W_tile_size_nof * 8 * 9 / 8;
        dory_dma_memcpy_async(&DMA_copy_W);
        dory_dma_barrier(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+108 + 48*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 29892;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+156 + 48*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 29948;
    ...
    k = (int32_t *) (l1_buffer + 29892);
    lambda = (int32_t *) (l1_buffer + 29948);
    W = (uint8_t *) (l1_buffer + 29776);
    y = (uint8_t *) (l1_buffer + 24008);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 12, 40, 12, 40, 12, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 24008);
