...
void gap8_BNReluConvolution6(
  ...
  DMA_copy_x.stride_2d = 800;
  DMA_copy_x.stride_1d = 20;
  DMA_copy_x.dir = 1;
  DMA_copy_x.tid = dory_dma_channel;
  
  DMA_copy_W.hwc_to_chw = 0;
  DMA_copy_W.stride_2d = 20;
  DMA_copy_W.stride_1d = 20;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      x_tile_size_nif = (_i_nif_load+1 == 1) ? 20 : 20;
      ...
      x_length_nif_byte = (_i_nif_load+1 == 1)   ? 20 : 20;
      ...
      W_tile_size_nif = (_i_nif_load+1 == 1) ? 20 : 20;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*8*1*1/8;
      W_length_nif_byte = (_i_nif_load+1 == 1) ? 20 : 20;
      // transfer of next input tile in double buffering
      if (_i_nif_load!=_i_nif_exec || _i_w_load!=_i_w_exec || _i_h_load!=_i_h_exec)
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 40, 22, 20, 40, 20,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 20, 1*1, 20, 1*1, 20, 0,0,0,0,0,0, 8);
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+400 + 80*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 35624;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+480 + 80*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 35712;
    ...
    k = (int32_t *) (l1_buffer + 35624);
    lambda = (int32_t *) (l1_buffer + 35712);
    W = (uint8_t *) (l1_buffer + 35216);
    y = (uint8_t *) (l1_buffer + 17608);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 40, 22, 20, 40, 20, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 17608);
