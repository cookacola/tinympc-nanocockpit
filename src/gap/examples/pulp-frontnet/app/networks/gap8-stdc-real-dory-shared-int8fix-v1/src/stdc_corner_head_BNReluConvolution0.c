...
void stdc_corner_head_BNReluConvolution0(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      x_tile_size_nif = (_i_nif_load+1 == 2) ? 16 : 16;
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 7 : 26;
      x_tile_size_w   = (_i_w_load+1 == 1)   ? 40 : 40;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
      x_length_nif_byte = (_i_nif_load+1 == 2)   ? 16 : 16;
      ...
      y_tile_size_nof = (_i_nof_load+1 == 2) ? 16 : 16;
      y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
      y_length_nof_byte = (_i_nof_load+1 == 2)   ? 16 : 16;
      W_tile_size_nof = (_i_nof_load+1 == 2) ? 16 : 16;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 26, 40, 16, 40, 32,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+288 + 64*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 32168;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+416 + 64*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 32240;
    ...
    k = (int32_t *) (l1_buffer + 32168);
    lambda = (int32_t *) (l1_buffer + 32240);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 24, 40, 16, 40, 32, 0, 0, 0, 0, 0, 0, 8);
