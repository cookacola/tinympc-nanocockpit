...
void BNReluConvolution5(
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+36864;
  DMA_copy_k.loc = (uint32_t) l1_buffer + 23832;
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+37376;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 24096;
  ...
  DMA_copy_x.number_of_2d_copies = 6;
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 5392) + 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_W.loc = (l1_buffer + 5392) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+36864 + 128*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 23832 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+37376 + 128*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 24096 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 23832 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 24096 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 5392 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 4616 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 64 : 64;
    x_tile_size_h_exec   = (_i_h_exec+1 == 1)   ? 6 : 6;
    x_tile_size_w_exec   = (_i_w_exec+1 == 3)   ? 3 : 6;
    y_tile_size_nof = (_i_nof_exec+1 == 4) ? 16 : 16;
    y_tile_size_h   = (_i_h_exec+1 == 1)   ? 6 : 6;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 6, 4, 16, 10, 64, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 4616) + db_y;
