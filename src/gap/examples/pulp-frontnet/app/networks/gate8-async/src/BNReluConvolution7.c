...
void BNReluConvolution7(
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+147456;
  DMA_copy_k.loc = (uint32_t) l1_buffer + 12360;
  DMA_copy_k.number_of_2d_copies = 1;
  DMA_copy_k.number_of_1d_copies = 1;
  DMA_copy_k.length_1d_copy = (uint16_t) 32;
  dory_dma_memcpy_async(&DMA_copy_k);
  dory_dma_barrier(&DMA_copy_k);


  DMA_copy_lambda.ext = (uint32_t) l2_W+148480;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 12432;
  DMA_copy_lambda.number_of_2d_copies = 1;
  DMA_copy_lambda.number_of_1d_copies = 1;
  DMA_copy_lambda.length_1d_copy = (uint16_t) 32;
  ...
  DMA_copy_x.number_of_2d_copies = 3;
  DMA_copy_x.number_of_1d_copies = 4;
  DMA_copy_x.length_1d_copy = 128;
  dory_dma_memcpy_async(&DMA_copy_x);
  dory_dma_barrier(&DMA_copy_x);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 3136) + 0;
  DMA_copy_W.number_of_2d_copies = 4;
  DMA_copy_W.number_of_1d_copies = 9;
  DMA_copy_W.length_1d_copy = 128;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 4, 3*3, 128, 3*3, 128, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 3136) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+147456 + 32*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 12360 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+148480 + 32*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 12432 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 12360 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 12432 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 3136 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 3080 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 128 : 128;
    x_tile_size_h_exec   = (_i_h_exec+1 == 1)   ? 3 : 3;
    x_tile_size_w_exec   = (_i_w_exec+1 == 3)   ? 2 : 4;
    y_tile_size_nof = (_i_nof_exec+1 == 32) ? 4 : 4;
    y_tile_size_h   = (_i_h_exec+1 == 1)   ? 3 : 3;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 3, 2, 4, 5, 128, 0, 0, 0, 0, 0, 0, 8);
