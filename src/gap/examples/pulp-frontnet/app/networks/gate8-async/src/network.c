...
/* VERBOSE disabled for deployment: it ran DORY per-layer golden checksum checks
 * plus printf each layer. On live camera input those mismatch by design, and the
 * print volume slows things and would pollute the gate8 UART on flash-boot. The
 * inference math is unaffected. */
// #define VERBOSE 1
...
static struct pi_cluster_task cluster_task_async;
static unsigned int args_async[5];
...
{
  args_async[0] = (unsigned int) l2_buffer;
  args_async[1] = (unsigned int) l2_buffer_size;
  args_async[2] = (unsigned int) l2_final_output;
