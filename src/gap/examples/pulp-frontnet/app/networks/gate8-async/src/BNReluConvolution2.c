...
void BNReluConvolution2(
  ...
  DMA_copy_k.loc = (uint32_t) l1_buffer + 24920;
  ...
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 25440;
  ...
  DMA_copy_x.number_of_2d_copies = 17;
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 6480) + 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      x_tile_size_w   = (_i_w_load+1 == 10)   ? 5 : 5;
      ...
      y_tile_size_w   = (_i_w_load+1 == 10)   ? 2 : 2;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 17, 5, 32, 40, 32,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_W.loc = (l1_buffer + 6480) + db_W;
    ...
    k = (int64_t *) (l1_buffer + 24920 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 25440 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 6480 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 5448 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 32 : 32;
    x_tile_size_h_exec   = (_i_h_exec+1 == 2)   ? 9 : 17;
    x_tile_size_w_exec   = (_i_w_exec+1 == 10)   ? 5 : 5;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 32 : 32;
    y_tile_size_h   = (_i_h_exec+1 == 2)   ? 4 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 10)   ? 2 : 2;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 2, 32, 20, 32, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 5448) + db_y;
