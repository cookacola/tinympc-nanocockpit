...
void espnet_gate_encoder_BNReluConvolution9(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 24, 22, 32, 40, 32,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+1024 + 256*_i_nof_load;
        ...
        DMA_copy_lambda.ext = (uint32_t) l2_W+1280 + 256*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 35104;
    ...
    lambda = (int64_t *) (l1_buffer + 35104);
    W = (uint8_t *) (l1_buffer + 33808);
    y = (uint8_t *) (l1_buffer + 16904);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 24, 22, 32, 40, 32, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 16904);
