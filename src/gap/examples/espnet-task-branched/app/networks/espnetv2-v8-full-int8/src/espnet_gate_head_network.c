...
#include "mem.h"
#include <string.h>
#include "espnet_gate_head_ReluPooling5.h"
#include "espnet_gate_head_ReluConvolution0.h"
#include "espnet_gate_head_ReluConvolution1.h"
#include "espnet_gate_head_ReluConvolution2.h"
#include "espnet_gate_head_ReluConvolution3.h"
#include "espnet_gate_head_ReluConvolution6.h"
#include "espnet_gate_head_ReluConvolution4.h"



#define L3_WEIGHTS_SIZE 327680
...
int espnet_gate_head_cycle_network_execution;
/* Moves the weights and the biases from hyperflash to hyperram */
void espnet_gate_head_network_initialize() {
...
void espnet_gate_head_execute_layer_fork(void *args) {
  ...
  {
    case 0:
      pi_cl_team_fork(NUM_CORES, (void *)espnet_gate_head_ReluConvolution0, args);
      break;
    case 1:
      pi_cl_team_fork(NUM_CORES, (void *)espnet_gate_head_ReluConvolution1, args);
      break;
    case 2:
      pi_cl_team_fork(NUM_CORES, (void *)espnet_gate_head_ReluConvolution2, args);
      break;
    case 3:
      pi_cl_team_fork(NUM_CORES, (void *)espnet_gate_head_ReluConvolution3, args);
      break;
    case 4:
      pi_cl_team_fork(NUM_CORES, (void *)espnet_gate_head_ReluConvolution4, args);
      break;
    case 5:
      pi_cl_team_fork(NUM_CORES, (void *)espnet_gate_head_ReluPooling5, args);
      break;
    case 6:
      pi_cl_team_fork(NUM_CORES, (void *)espnet_gate_head_ReluConvolution6, args);
      break;
  }
...
struct espnet_gate_head_network_run_token espnet_gate_head_network_run_async(void *l2_buffer, size_t l2_buffer_size, void *l2_final_output, int exec, int initial_dir)
{
  ...
  // open cluster...
  pi_cluster_task(&cluster_task, espnet_gate_head_network_run_cluster, args);
  ...
  return (struct espnet_gate_head_network_run_token) {
    .cluster_dev = cluster_dev
  };
}

void espnet_gate_head_network_run_wait(struct espnet_gate_head_network_run_token token)
{
  pi_cluster_close(&token.cluster_dev);
  print_perf("Final", espnet_gate_head_cycle_network_execution, 37786080);
}

void espnet_gate_head_network_run(void *l2_buffer, size_t l2_buffer_size, void *l2_final_output, int exec, int initial_dir)
{
  espnet_gate_head_network_run_wait(espnet_gate_head_network_run_async(l2_buffer, l2_buffer_size, l2_final_output, exec, initial_dir));
}
...
static struct pi_cluster_task espnet_gate_head_cluster_task_async;
static unsigned int espnet_gate_head_args_async[5];
void espnet_gate_head_network_run_async_cl(void *l2_buffer, size_t l2_buffer_size, void *l2_final_output, int exec, int initial_dir, pi_device_t *cluster, pi_task_t *network_done)
{
  espnet_gate_head_args_async[0] = (unsigned int) l2_buffer;
  espnet_gate_head_args_async[1] = (unsigned int) l2_buffer_size;
  espnet_gate_head_args_async[2] = (unsigned int) l2_final_output;
  espnet_gate_head_args_async[3] = (unsigned int) exec;
  espnet_gate_head_args_async[4] = (unsigned int) initial_dir;
  pi_cluster_task(&espnet_gate_head_cluster_task_async, espnet_gate_head_network_run_cluster, espnet_gate_head_args_async);
  espnet_gate_head_cluster_task_async.stack_size = 3500;
  espnet_gate_head_cluster_task_async.slave_stack_size = 3400;
  pi_cluster_send_task_to_cl_async(cluster, &espnet_gate_head_cluster_task_async, network_done);
}

void espnet_gate_head_network_run_cluster(void *args) {
...
/* ---------------------------------- */
  ...
  espnet_gate_head_cycle_network_execution = 0;
/* MAIN SECTION
...
*/
    ...
    pi_perf_stop();
    pi_perf_start();
    espnet_gate_head_execute_layer_fork((void *) &largs);
    // performance measurements: end
    pi_perf_stop();
    perf_cyc =  pi_perf_read(PI_PERF_CYCLES);
    espnet_gate_head_cycle_network_execution += perf_cyc;
