...
void espnet_gate_encoder_BNReluConvolution1(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  im2col = l1_buffer + 33720;
  volatile uint8_t *pwt_buffer;
  pwt_buffer = im2col + 408;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 7)   ? 9 : 14;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 7)   ? 8 : 12;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 14, 80, 16, 80, 16,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+144 + 128*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 33448;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+272 + 128*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 33584;
    ...
    k = (int64_t *) (l1_buffer + 33448);
    lambda = (int64_t *) (l1_buffer + 33584);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 12, 80, 16, 80, 16, 0, 0, 0, 0, 0, 0, 8);
